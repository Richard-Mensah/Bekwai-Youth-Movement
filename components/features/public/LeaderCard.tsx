...
import type { Leader } from "@/constants/leadership"
...
/** A photo-ready person card. Falls back to initials / icon before appointment. */
export default function LeaderCard({ leader }: { leader: Leader }) {
  const { name, photo, title, ukEquivalent, blurb, sdg } = leader
    ...
    <div className="flex h-full flex-col rounded-2xl border border-canopy/10 bg-white p-5 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
      <div className="flex items-center gap-4">
        <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full ring-2 ring-gold-400/40">
          {photo ? (
            <Image src={photo} alt={name ?? title} fill className="object-cover" />
          ) : name ? (
        ...
        <div className="min-w-0">
          ...
          {ukEquivalent && (
            <p className="text-xs text-ink/45">{ukEquivalent}</p>
          )}
        </div>
      </div>
