        ...
        <Reveal>
          ...
          <div className="mt-7 space-y-3">
            {SEATS.map(({ icon: Icon, title, age, body }) => (
              <div
                key={title}
                className="flex gap-4 rounded-2xl border border-canopy/10 bg-white p-4 shadow-card dark:border-white/10 dark:bg-canopy-800"
              >
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm text-ink/60 dark:text-paper/60">{body}</p>
                </div>
              </div>
            ))}
          </div>
