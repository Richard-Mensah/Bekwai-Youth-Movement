          ...
          return (
            <div key={status} className="w-48 shrink-0">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="rounded-full bg-canopy-50 px-1.5 text-[11px] font-medium text-ink/55">
                  {col.length}
                </span>
              </div>
              <div className="space-y-2">
                {col.map((p) => (
                  <Link
                    ...
                    className="block rounded-lg border border-canopy/10 bg-white p-3 shadow-sm transition hover:border-brand-green"
                  >
                    <p className="text-xs font-medium leading-snug text-ink/75">
                      {p.name}
                    </p>
                    <p className="mt-1 text-[11px] text-ink/40">{p.communityName}</p>
                ...
                {col.length === 0 && (
                  <div className="rounded-lg border border-dashed border-canopy/10 p-3 text-center text-[11px] text-ink/30">
                    Empty
                  </div>
                )}
              </div>
