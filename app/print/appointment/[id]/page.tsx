...
export default async function AppointmentLetter({
  ...
  return (
    <main className="min-h-screen bg-canopy-50 py-8 print:bg-white print:py-0">
      ...
      <article className="mx-auto max-w-3xl bg-white p-12 shadow-lg print:max-w-none print:p-10 print:shadow-none">
        <header className="flex items-center gap-4 border-b-2 border-brand-green pb-5">
          ...
          <div>
            ...
            <p className="text-xs text-ink/55">{ORG.assembly} · {ORG.region}</p>
          </div>
          <p className="ml-auto text-right text-xs text-ink/40">
            Ref: BYM-LOA-{officer.id.slice(0, 6).toUpperCase()}
          </p>
        </header>

        <p className="mt-8 text-sm text-ink/55">{formatDate(new Date())}</p>

        <h2 className="mt-6 text-center font-serif text-lg font-bold uppercase tracking-wide text-brand-green-700">
          Letter of Appointment
        </h2>

        <div className="mt-8 space-y-4 text-sm leading-relaxed text-canopy">
        ...
        <div className="mt-16 grid grid-cols-2 gap-10 text-sm">
          <div>
            ...
            <p className="mt-1 font-semibold text-ink/75">Director-General</p>
            <p className="text-xs text-ink/55">BYM Youth General Assembly</p>
          </div>
          <div>
            <div className="h-12 border-b border-gray-400" />
            <p className="mt-1 font-semibold text-ink/75">Secretary-General</p>
            <p className="text-xs text-ink/55">Custodian of Records</p>
          </div>
        </div>

        <footer className="mt-12 border-t border-canopy/10 pt-4 text-center text-[11px] text-ink/40">
